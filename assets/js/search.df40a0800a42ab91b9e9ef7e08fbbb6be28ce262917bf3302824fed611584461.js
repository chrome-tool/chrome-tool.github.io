  ...
  if (fuse) {
    ...
    if (results.length !== 0) {
      ...
      for (let item in results) {
        resultSet.push(`
                        <div class="border text-card-foreground flex flex-col gap-2 p-4 rounded-lg bg-background shadow-sm hover:shadow-md transition-shadow">
                            <a href="${results[item].item.permalink}" aria-label="${results[item].item.title}">
                                <img src="/placeholder.svg" width="150" height="150" alt="Extension" class="mx-auto rounded-md" style="aspect-ratio: 150 / 150; object-fit: cover"/>
                                <div class="flex flex-col gap-1" data-id="56">
                                <h3 class="text-lg font-bold" data-id="57">${results[item].item.title}</h3>
                                <p class="text-muted-foreground line-clamp-2" data-id="58">
      ...
      resList.innerHTML = resultSet.join('');
